...
object LoanDetailsDestination : NavigationDestination {
    override val route = "loan_details"
    override val titleRes = R.string.title_loan_details
    const val loanIdArg = "loanId"
    val routeWithArgs = "$route/{${loanIdArg}}"
}
...
fun LoanDetailsScreen(
    ...
    viewModel: LoanDetailsViewModel = viewModel(factory = AppViewModelProvider.Factory)
){
    ...
    Scaffold(
        topBar = {
            GestionPrestamoTopAppBar(
                title = stringResource(LoanDetailsDestination.titleRes),
                canNavigateBack = true,
                navigateUp = navigateBack
            )
        }, floatingActionButton = {
            FloatingActionButton(
                onClick = { navigateToEditClient(uiState.value.loanDetails.id)  },
        ...
        UserDetailsBody(
            PrestamoDetailsUiState = uiState.value ,
            onDelete = {
...
private fun UserDetailsBody(
    PrestamoDetailsUiState: PrestamoDetailsUiState,
    onDelete: () -> Unit,
    modifier: Modifier = Modifier
) {
    Column(
        ...
        Column(
            ...
            LoanDetails(
                loan = PrestamoDetailsUiState.loanDetails.toEntity(), modifier = Modifier.fillMaxWidth()
            )
            OutlinedButton(
    ...
    Card(
        ...
        Column(
            ...
            ClientDetailsRow(
                ...
                clientDetail = loan.idPrestamista.toString(),
            ...
            ClientDetailsRow(
                ...
                clientDetail = loan.valor.toString(),
                modifier = Modifier.padding(
                    horizontal = dimensionResource(id = R.dimen.padding_medium)
                )
            )
            ClientDetailsRow(
                ...
                clientDetail = loan.plazo.toString(),
                modifier = Modifier.padding(
                    horizontal = dimensionResource(id = R.dimen.padding_medium)
                )
            )
